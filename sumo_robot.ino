...
void stopmotors(){
  digitalWrite(in1,0);
  digitalWrite(in2,0);  // ضيفتها عشان احطها في شرط الif بدل اني اصفر السرعه
  digitalWrite(in3,0);
  digitalWrite(in4,0);
}
...
void attack(){
  analogWrite(ena, 255);
  analogWrite(enb, 255);  // هو كدا كدا هيبقي اقل من 255 وبعدين احنا عايزينه يهجم باقصي قوته ف ملهاش لازمه الزياده التدريجيه 
...
void moveright(){
  ...
  digitalWrite(in1, 0);
  digitalWrite(in2, 1);
  digitalWrite(in3, 1);  // كنت عاكس هنا عامله يرجع لورا
  digitalWrite(in4, 0);
...
void setup() {
  ...
  pinMode(in3,OUTPUT);
  pinMode(in4,OUTPUT);
  pinMode(ena,OUTPUT);  //بتعرفهم بردو عشان بيخرجوا السرعه اللي كاتبها للدرايفر
  pinMode(enb,OUTPUT);
...
void loop() {
  ...
  if(irR==1 ){
    ...
    stopmotors(); // بدل تصفير السرعه عشان لغيتها
    moveright();
  }}
    ...
    attack(); //ده السبب اللي انا عامله عشانه عشان لو القرايه اختفت فجاه بعد ما كان قاري انه قدامه فعلا ف كده الاختفاء ده مش صح والروبوت قدامه ف يكمل هجوم
     ...
     moveforward();  // مكنش ليها لازمه اعتقد ف شيلتها
  }
  else  {
    search();
  }
